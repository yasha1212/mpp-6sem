...
async function fetchContainers() {
    socket.emit("askContainers", getContainersQuery());
}

async function createContainer() {
    socket.emit("createContainer", createContainerQuery());
}

async function createTask(containerId, input, file) {
    ...
    //     body: formData
    // });
    socket.emit("createTask", createTaskQuery(), {containerId, input});
}

async function deleteContainer(id) {
    socket.emit("deleteContainer", deleteContainerQuery(), {id});
}

async function deleteTask(containerId, taskId) {
    socket.emit("deleteTask", deleteTaskQuery(), {containerId, taskId});
}

async function editContainer(id, container) {
    socket.emit("editContainer", updateContainerQuery(), {id, container});
}

async function editTask(containerId, taskId, input) {
    socket.emit("editTask", updateTaskQuery(), {containerId, taskId, input});
}
